...
test.deeplearning.mojo <-
  function() {
    ...
    e <- tryCatch({
      ...
      compareFrames(twoFrames$h2oPredict,twoFrames$mojoPredict, tolerance = 1e-4)
    }, error = function(x) x)
    if (!is.null(e) && (!all(sapply("DistributedException", grepl, e[[1]]))))
      FAIL(e)   # throw error unless it is unstable model error.
  }

compareFrames <- function(frame1, frame2, tolerance=1e-6) {
  expect_true(nrow(frame1) == nrow(frame2), info="H2O predict and mojo predict frame do not have equal numbers of rows.")
  expect_true(ncol(frame1) == ncol(frame2), info="H2O predict and mojo predict frame do not have equal numbers of columns")
  for (rowInd in range(1,nrow(frame1))) {
    for (colInd in range(1, ncol(frame1))) {
      expect_true(abs(frame1[rowInd, colInd]-frame2[rowInd, colInd])< tolerance, info="H2O predict != mojo predict ")
    }
  }
}
...
setParmsData <- function(numTest=1000) {
  ...
  if ((problem == 'regression') || enableAutoEncoder) {
    loss <- 'Automatic'
  } else {
    loss <- 'CrossEntropy'
  }
